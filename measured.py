...
Measured = np.load('motionTrajector.npy')
...
plt.plot(MarkedMeasure[:,0],MarkedMeasure[:,1],'og',label='trajectoire mésurée')
plt.axis([0,384,288,0])
#plt.hold(True)
plt.plot(filtered_state_means[:,0],filtered_state_means[:,1],'xb',label='trajectoire prédite')
plt.legend(loc=2)
plt.title("Sortie Filtre de Kalman")
plt.savefig("outputkalmanfilter.png")
plt.show()